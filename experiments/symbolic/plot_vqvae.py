...
Reads eval JSONs from outputs/vqvae/<model>/vqvae_results/codebook_K{K}.eval.json
...
#   outputs/vqvae/<model>/baselines.json
# containing k0_accuracy and unquantized_accuracy.

def load_baselines(base_path: Path) -> dict:
    """
    ...
    for model_name in MODELS:
        bl_path = base_path / model_name / "baselines.json"
...
def load_eval_results(base_path: Path) -> dict:
    ...
    for model_name in MODELS:
        model_results = {}
        results_dir = base_path / model_name / "vqvae_results"
...
def main():
    ...
    if not baselines:
        ...
        print("[WARN] Run sanity_check_vqvae.py first to generate baselines.json files.")
