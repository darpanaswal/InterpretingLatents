...
Usage:
    python sanity_check_vqvae.py --model coconut_u --max_instances 50
"""
...
from utils.config import (
    BASE_DIR, BASE_GPT2, COCONUT_GPT2, COCONUT_GPT2_U, PAUSE_GPT2, PROSQA_TEST, CONTROL_EXPT
)
...
@torch.no_grad()
def run_normal_inference(base_model, tokenizer, end_id, sample, n_thoughts, device):
    """
    Normal (unquantized) Coconut inference: manual recurrence + greedy decode.
    This is what our VQ-VAE eval does, minus the quantization step.
    """
    ...
    correct = sample.get("answer", "").replace(",", "").strip()
    return answer, answer == correct, text


def check_identity_codebook(base_model, tokenizer, end_id, data, n_thoughts, device):
    """
    Compare normal inference to the Coconut wrapper's generate().
    If they match, our manual recurrence + decoding is correct.
    """
    print("\n" + "=" * 60)
    print("CHECK 1: Identity codebook (normal inference accuracy)")
    print("=" * 60)
    ...
    for idx, sample in enumerate(data):
        answer, is_correct, full_text = run_normal_inference(
...
def main():
    ...
    parser.add_argument(
        ...
        help="Base output dir. Default: BASE_DIR/outputs/vqvae/",
    )
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    args = parser.parse_args()

    prosqa_path = args.prosqa_path or str(PROSQA_TEST)
    output_base = Path(args.output_dir) if args.output_dir else BASE_DIR / "outputs" / "vqvae"
    ...
    for model_name in args.models:
        ...
        # Check 2: Position ID audit (on first instance)
        check_position_ids(base_model, tokenizer, data[0], args.n_thoughts, args.device)

        # Check 3: Hidden state identity (on first instance)
        ...
        accuracy_k0, _ = check_identity_codebook(
            base_model, tokenizer, end_id, data, 0, args.device
        )
        ...
        # ── Save baselines JSON ─────────────────────────────────────
        # Saved to outputs/vqvae/<model>/baselines.json
        ...
        baselines_dir = output_base / model_name
